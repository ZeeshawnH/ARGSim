...
#include <random>
using rng_uniform_int_t = std::uniform_int_distribution<>;
using rng_uniform_real_t = std::uniform_real_distribution<>;
using rng_exponential_t = std::exponential_distribution<>;
using rng_binomial_t = std::binomial_distribution<>;
...
/**
 * Functions the same as above method without using boost libraries
 * @author Zeeshawn Hasnain
 */
template<class PRNG, class Dist>
inline typename Dist::result_type make_gen(PRNG& rng, Dist d)
{
    return d(rng);
}
